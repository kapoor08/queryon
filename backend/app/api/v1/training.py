...
async def start_training(
    ...
    try:
        ...
        widget = (
            db.query(Widget)
            .filter(Widget.id == request.widget_id, Widget.user_id == user.id)
            .first()
        )
        ...
        if max_documents != -1 and len(request.training_data) > max_documents:
            raise SubscriptionError(
                f"Document limit exceeded. Your plan allows {max_documents} documents."
            )

        # Start training
        result = await training_service.start_training(
            widget_id=request.widget_id, training_data=request.training_data, db=db
        )
...
async def get_training_status(
    widget_id: str, api_key: str, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        widget = (
            db.query(Widget)
            .filter(Widget.id == widget_id, Widget.user_id == user.id)
            .first()
        )
        ...
        training_data = [
            {
                ...
                "metadata": {"file_size": file.size, "file_name": file.filename},
            }
        ]

        # Start training with uploaded file
        result = await training_service.start_training(
            widget_id=widget_id, training_data=training_data, db=db
        )
...
async def delete_training_document(
    document_id: str, widget_id: str, api_key: str, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        success = await training_service.delete_training_document(
            document_id=document_id, widget_id=widget_id, db=db
        )
